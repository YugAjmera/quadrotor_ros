...
void RosThread::run()
{
    ...
    vel_pub	   = nh_.advertise<geometry_msgs::Twist>(nh_.resolveName("quadrotor/cmd_vel"),1);
    vel_sub	   = nh_.subscribe(nh_.resolveName("quadrotor/cmd_vel"),50, &RosThread::velCb, this);
    ...
    navdata_sub	   = nh_.subscribe(nh_.resolveName("quadrotor/ardrone/navdata"),50, &RosThread::navdataCb, this);
    joy_sub	   = nh_.subscribe(nh_.resolveName("joy"),50, &RosThread::joyCb, this);

    takeoff_pub	   = nh_.advertise<std_msgs::Empty>(nh_.resolveName("quadrotor/ardrone/takeoff"),1);
    land_pub	   = nh_.advertise<std_msgs::Empty>(nh_.resolveName("quadrotor/ardrone/land"),1);
    toggleState_pub	   = nh_.advertise<std_msgs::Empty>(nh_.resolveName("quadrotor/ardrone/reset"),1);

    takeoff_sub	   = nh_.subscribe(nh_.resolveName("quadrotor/ardrone/takeoff"),1, &RosThread::takeoffCb, this);
    land_sub	   = nh_.subscribe(nh_.resolveName("quadrotor/ardrone/land"),1, &RosThread::landCb, this);
    toggleState_sub	   = nh_.subscribe(nh_.resolveName("quadrotor/ardrone/reset"),1, &RosThread::toggleStateCb, this);

    toggleCam_srv        = nh_.serviceClient<std_srvs::Empty>(nh_.resolveName("quadrotor/ardrone/togglecam"),1);
    flattrim_srv         = nh_.serviceClient<std_srvs::Empty>(nh_.resolveName("quadrotor/ardrone/flattrim"),1);
